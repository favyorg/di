...
test('Custom module(ask gen deps)', () => {
  const Module = makeModule({
    transformOutput: (res, deps, isRoot) => {
      ...
      type Gen<A extends PropertyKey, RES, DEPS, P, C> = RES extends Generator<
        infer Y,
        infer R
      >
        ? TModule<A, R, UnionToIntersection<Y>>
        : TModule<A, RES, DEPS>;
  ...
  type Tag<N extends PropertyKey, R> = Generator<
    Live<TModule<N, R, unknown>>,
    R
  >;
