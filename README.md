...
// You need to specify the dependencies that all modules require
...
// To start, you must provide all dependencies of the right type for all modules and submodules
...
```
An easy replacement of dependencies.
```ts
...
```
Dependencies can be of any type, the module can return any value.
```ts
...
```
### Services
The service is called only once, no matter where it is called.
